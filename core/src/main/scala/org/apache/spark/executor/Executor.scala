...
import org.apache.spark.util.{Stats, SparkUncaughtExceptionHandler, AkkaUtils, Utils}
  ...
  class TaskRunner(
    ...
    override def run() {
      ...
      try {
        ...
        val directResult = if(Stats.statistics.get() == null){
          new DirectTaskResult(valueBytes, accumUpdates, task.metrics.orNull)
        }else{
          new DirectTaskResult(valueBytes, accumUpdates, task.metrics.orNull, Some(Stats.statistics.get()))
        }
