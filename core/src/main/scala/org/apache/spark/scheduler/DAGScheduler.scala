...
class DAGScheduler(
  ...
  //zengdan
  private[scheduler] val queryStatistics = Map[Int, Array[Long]]()
  ...
  def taskEnded(
      ...
      taskStats: Option[Map[Int,Array[Int]]] = None) {
    eventProcessActor ! CompletionEvent(task, reason, result, accumUpdates, taskInfo, taskMetrics, taskStats)
  }
  ...
  //zengdan update application_local statistics
  private[scheduler] def updateStatistics(taskStats: Map[Int, Array[Int]]): Unit = {
      //need sychronized??
      for((key,value) <- taskStats){
        val old = queryStatistics.get(key).getOrElse(Array(0L,0L))
        old(0) += value(0).toLong
        old(1) += value(1).toLong
        queryStatistics.update(key, old)
      }
  }

  //zengdan get statistics
   def getStatistics = queryStatistics
   def clearStatistics = queryStatistics.clear()
  ...
  private[scheduler] def handleTaskCompletion(event: CompletionEvent) {
    ...
    val stage = stageIdToStage(task.stageId)
    //stage.rdd.asInstanceOf[SchemaRDD].
...
private[scheduler] class DAGSchedulerEventProcessActor(dagScheduler: DAGScheduler)
  ...
  def receive = {
    ...
    case completion @ CompletionEvent(task, reason, _, _, taskInfo, taskMetrics, taskStatistics) =>
