...
class DirectTaskResult[T](var valueBytes: ByteBuffer, var accumUpdates: Map[Long, Any],
    var metrics: TaskMetrics, var statistics: Option[Map[Int, Array[Int]]] = None)
  ...
  override def writeExternal(out: ObjectOutput): Unit = Utils.tryOrIOException {
    ...
    if(statistics.isDefined){
      val sts = statistics.get
      out.writeByte(sts.size)
      for ((key, value) <- sts) {
        out.writeInt(key)
  ...
  override def readExternal(in: ObjectInput): Unit = Utils.tryOrIOException {
    ...
    if(numStats == 0){
      ...
      val statsMap = Map[Int, Array[Int]]()
      for (i <- 0 until numStats) {
        val key = in.readInt()
        statsMap(key) = Array(in.readInt(), in.readInt())
      }
      statistics = Some(statsMap)
    }
