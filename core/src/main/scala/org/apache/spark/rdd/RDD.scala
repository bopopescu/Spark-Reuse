...
abstract class RDD[T: ClassTag](
  ...
  var cacheID: Option[Int] = None //zengdan

  //zengdan
  def setContextAndDep(other: RDD[_]) = {
    this.sc = other.sc
  ...
  def persist(newLevel: StorageLevel): this.type = {
    ...
    if(!this.cacheID.isDefined)  //zengdan
  ...
  //zengdan
  /*
  def loadPartitions[U: ClassTag](
      pars: Array[Partition]): RDD[U] = {

    new LoadPartitionsRDD(sc, pars)
  }
  */
  ...
  /**
   * zengdan
   * clear rdd dependencies whose data is in tachyon, in order to avoid unnecessary map stage
   */
  def clearGlobalDependencies(): Unit = {
    for(partition <- partitions) {
      val blockId = RDDBlockId(id, partition.index, cacheID)
      if(!SparkEnv.get.blockManager.tachyonStore.checkGlobalExists(blockId)){
        return
      }
    }
    dependencies_ = Nil
    deps = Nil
    //update blockinfo related information
    for(partition <- partitions) {
      val blockId = RDDBlockId(id, partition.index, cacheID)
      SparkEnv.get.blockManager.doPutInfo(blockId)
    }
