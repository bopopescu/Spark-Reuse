...
private[spark] class BlockManager(
  ...
  //zengdan doPut only update blockinfo information
  def doPutInfo(
      blockId: BlockId,
      tellMaster: Boolean = true)
  : Unit = {
     ...
     * not be able to get() this block until we call markReady on its BlockInfo. */
    val putBlockInfo = {
      val tinfo = new BlockInfo(StorageLevel.OFF_HEAP, tellMaster)
      // Do atomically !
      val oldBlockOpt = blockInfo.putIfAbsent(blockId, tinfo)
      if (oldBlockOpt.isDefined) {
        if (oldBlockOpt.get.waitForReady()) {
    ...
    putBlockInfo.synchronized {
      ...
      //var marked = false
      try {
        //size = tachyonStore.getSize(blockId)

        val putBlockStatus = getCurrentBlockStatus(blockId, putBlockInfo)
         //marked = true
         putBlockInfo.markReady(putBlockStatus.tachyonSize)
         if (tellMaster) {
           reportBlockStatus(blockId, putBlockInfo, putBlockStatus)

          }
      } catch {
        case e: Exception => logError("Faild to load tachyon block information")
                             System.exit(-1)
