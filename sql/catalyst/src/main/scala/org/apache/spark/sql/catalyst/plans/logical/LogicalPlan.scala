...
abstract class LogicalPlan extends QueryPlan[LogicalPlan] with Logging with Serializable{
  ...
  //zengdan
  def operatorMatch(plan: LogicalPlan):Boolean = {
    (plan.getClass == this.getClass) && compareExpression(plan.expressions, this.expressions)
  }

  def compareExpression(expr1: Seq[Expression], expr2: Seq[Expression]): Boolean ={
    val e1 = expr1.toArray.sortWith(_.compareTo(_))
    val e2 = expr2.toArray.sortWith(_.compareTo(_))
    ...
    if(e1.length != e2.length)
      ...
      var i = 0
      while(i < e1.length){
        val x = e1(i).treeStringByName
        val y = e2(i).treeStringByName
        if(x.compareTo(y) != 0)
          return false
        i += 1
      }
  ...
  //zengdan delete protected
  @transient lazy val cleanArgs: Seq[Any] = {
