...
abstract class TreeNode[BaseType <: TreeNode[BaseType]] {
  ...
  //zengdan
  //in case of more than 2 children
  def argStringMinusChildren: String = productIterator.flatMap {
    case tn: TreeNode[_] if children contains tn => Nil
    case tn: TreeNode[_] if tn.toString contains "\n" => s"(${tn.simpleString})" :: Nil
    case seq: Seq[_] =>
      seq.foldLeft(new StringBuilder("["))((str, tn) =>
        if(!(children contains tn)){str.append(tn).append(",")} else str)
      .append("]")
      //seq.mkString("[", ",", "]") :: Nil
    case set: Set[_] =>
      set.foldLeft(new StringBuilder("{"))((str, tn) =>
        if(!(children contains tn)){str.append(tn).append(",")} else str)
        .append("}")
      //set.mkString("{", ",", "}") :: Nil
    case other => other :: Nil
  ...
  //zengdan
  protected def generateTreeStringByName(depth: Int, builder: StringBuilder): StringBuilder = {
    builder.append(" " * depth)
    builder.append(simpleStringByName)
    builder.append("\n")
    children.foreach(_.generateTreeStringByName(depth + 1, builder))
    builder
  }
