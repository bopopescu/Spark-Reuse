...
trait ScalaReflection {
  ...
  //zengdan
  def convertRowToScalaByName(r: Row, output: Seq[Attribute]): Row = {
    val schema = StructType.fromAttributes(output)
    val x = new GenericRow(
      r.zip(schema.fields.map(_.dataType))
        .map(r_dt => convertToScala(r_dt._1, r_dt._2))
        .zip(output.map(_.name)).sortWith((r1,r2) => r1._2.compareTo(r2._2) < 0)
        .map(_._1).toArray)
    x
  }
