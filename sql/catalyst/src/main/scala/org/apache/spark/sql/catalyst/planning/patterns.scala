...
object PhysicalOperation extends PredicateHelper {
  ...
  def unapply(plan: LogicalPlan): Option[ReturnType] = {
    ...
    Some((fields.getOrElse(child.output), filters, child)) //zengdan
  }
    ...
    case logical.Aggregate(groupingExpressions, aggregateExpressions, child) =>
      ...
      if (allAggregates.size == partialAggregates.size) {
        ...
        val namedGroupingExpressions: Map[Expression, NamedExpression] = groupingExpressions.map {
          ...
          case other => (other, Alias(other, "PartialGroup")())   //group by nothing ?
        }.toMap
...
object ExtractEquiJoinKeys extends Logging with PredicateHelper {
  ...
  //zengdan nodeRef
  type ReturnType =
    (JoinType, Seq[Expression], Seq[Expression], Option[Expression], LogicalPlan, LogicalPlan, Option[QNodeRef])
  ...
  def unapply(plan: LogicalPlan): Option[ReturnType] = plan match {
    case join @ Join(left, right, joinType, condition) =>
      ...
      if (joinKeys.nonEmpty) {
        ...
        //zengdan nodeRef
        Some((joinType, leftKeys, rightKeys, otherPredicates.reduceOption(And), left, right, plan.nodeRef))
      } else {
        None
      }
    case _ => None
