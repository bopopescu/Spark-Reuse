...
private[sql] abstract class SparkStrategies extends QueryPlanner[SparkPlan] {
  ...
  object LeftSemiJoin extends Strategy with PredicateHelper {
    def apply(plan: LogicalPlan): Seq[SparkPlan] = plan match {
      ...
      case ExtractEquiJoinKeys(LeftSemi, leftKeys, rightKeys, condition, left, right, nodeRef) =>
  ...
  object HashJoin extends Strategy with PredicateHelper {
    ...
    private[this] def makeBroadcastHashJoin(
      ...
      val broadcastHashJoin = execution.joins.BroadcastHashJoin(
        leftKeys, rightKeys, side, planLater(left), planLater(right), optionRef)
      condition.map(Filter(_, broadcastHashJoin)).getOrElse(broadcastHashJoin) :: Nil
    }

    def apply(plan: LogicalPlan): Seq[SparkPlan] = plan match {
      case ExtractEquiJoinKeys(Inner, leftKeys, rightKeys, condition, left, right, nodeRef)
        if sqlContext.autoBroadcastJoinThreshold > 0 &&
           right.statistics.sizeInBytes <= sqlContext.autoBroadcastJoinThreshold =>
        makeBroadcastHashJoin(leftKeys, rightKeys, left, right, condition, joins.BuildRight, nodeRef)

      case ExtractEquiJoinKeys(Inner, leftKeys, rightKeys, condition, left, right, nodeRef)
        if sqlContext.autoBroadcastJoinThreshold > 0 &&
           left.statistics.sizeInBytes <= sqlContext.autoBroadcastJoinThreshold =>
          makeBroadcastHashJoin(leftKeys, rightKeys, left, right, condition, joins.BuildLeft, nodeRef)
      ...
      case ExtractEquiJoinKeys(Inner, leftKeys, rightKeys, condition, left, right, nodeRef) =>
        ...
        val hashJoin = joins.ShuffledHashJoin(
          leftKeys, rightKeys, buildSide, planLater(left), planLater(right), nodeRef)
        condition.map(Filter(_, hashJoin)).getOrElse(hashJoin) :: Nil

      case ExtractEquiJoinKeys(joinType, leftKeys, rightKeys, condition, left, right, nodeRef) =>
        joins.HashOuterJoin(
    ...
    def apply(plan: LogicalPlan): Seq[SparkPlan] = plan match {
      case PhysicalOperation(projectList, filters, mem: InMemoryRelation) =>
        pruneFilterProject(
          ...
          InMemoryColumnarTableScan(_,  filters, mem),
          plan.nodeRef) :: Nil
      case _ => Nil
    }
  }
    ...
    def apply(plan: LogicalPlan): Seq[SparkPlan] = plan match {
      case logical.Distinct(child) =>
        execution.Distinct(partial = false,
          execution.Distinct(partial = true, planLater(child), plan.nodeRef), plan.nodeRef) :: Nil
      ...
      case logical.Aggregate(group, agg, child) =>
        execution.Aggregate(partial = false, group, agg, planLater(child), plan.nodeRef) :: Nil
      ...
      case LogicalRDD(output, rdd) => PhysicalRDD(output, rdd, plan.nodeRef) :: Nil
      case _ => Nil
    }
  }
