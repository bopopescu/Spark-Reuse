 ...
 */
private[spark] class QGMasterArguments(args: Array[String], conf: SparkConf) {
  ...
  if (System.getenv("SPARK_QGMASTER_WEBUI_PORT") != null) {
    webUiPort = System.getenv("SPARK_QGMASTER_WEBUI_PORT").toInt
  }
  ...
  if (conf.contains("spark.qgmaster.ui.port")) {
    webUiPort = conf.get("spark.qgmaster.ui.port").toInt
  }
