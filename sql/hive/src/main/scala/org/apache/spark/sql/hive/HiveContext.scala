...
import org.apache.spark.sql.execution.{Command => PhysicalCommand, LogicalRDD, ExtractPythonUdfs, QueryExecutionException}
...
class HiveContext(sc: SparkContext) extends SQLContext(sc) {
  ...
  val hivePlanner = new SparkPlanner with HiveStrategies {
    ...
    override val strategies: Seq[Strategy] = extraStrategies ++ Seq(
      ...
      HiveTableScans,
      DataSinks,   //zengdan  no need to collect info and reuse data
      Scripts,
      HashAggregation,
      LeftSemiJoin,
      HashJoin,
