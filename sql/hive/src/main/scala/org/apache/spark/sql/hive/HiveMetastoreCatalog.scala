...
private[hive] case class MetastoreRelation
  ...
  @transient lazy val hiveQlPartitions = partitions.map { p =>
    new Partition(hiveQlTable, p)
  }


  //zengdan  delete lazy and transient
  //@transient
    ...
    sizeInBytes = {
      ...
      BigInt(
        ...
        Option(totalSize).map(_.toLong).filter(_ > 0)
          ...
          //.getOrElse((10*1024*124+1).toLong)))
  ...
  override def operatorMatch(plan: LogicalPlan): Boolean = {
    if(plan.isInstanceOf[MetastoreRelation]){
      val ptable = plan.asInstanceOf[MetastoreRelation]
      this.databaseName == ptable.databaseName && this.tableName == ptable.tableName
    }else{
      false
    }
  }
